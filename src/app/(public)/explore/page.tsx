...
import { Search, MapPin, Star, Filter, SlidersHorizontal, Loader2 } from "lucide-react";
...
function ExploreContent() {
  ...
  // ✅ Effect: Sync URL with State (Debounced)
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      // Push to URL without scrolling to top
  ...
  return (
    <div className="container py-20 px-8 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end gap-4 mb-8 border-b pb-6">
        <div>
          ...
          <p className="text-slate-500 mt-2">
            {loading ? "Searching..." : `Found ${listings.length} experiences for you.`}
          </p>
        </div>
      </div>
          ...
          {loading ? (
              ...
              {listings.map((tour) => (
                <div key={tour.id} className="relative group">
                      ...
                      <CardContent className="p-5 flex-1 flex flex-col">
                        ...
                        <div className="flex items-center gap-1 mb-4">
                            ...
                            <span className="text-sm font-bold text-slate-900">
                               {getRating(tour.reviews)}
                            </span>
                            <span className="text-xs text-slate-500">
                               ({tour._count?.reviews || tour.reviews?.length || 0} reviews)
                            </span>
                        </div>
                  ...
                  <div className="absolute top-3 right-3 z-20">
                     <div className="bg-white/20 backdrop-blur-sm rounded-full p-0.5 hover:bg-white transition-colors cursor-pointer">
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center border-2 border-dashed border-slate-200 rounded-2xl bg-slate-50/50">
              ...
              <p className="text-slate-500 mt-2 max-w-xs mx-auto">
                 We couldn't find any tours matching your filters. Try searching for something else.
              </p>
              <Button 
                variant="link" 
